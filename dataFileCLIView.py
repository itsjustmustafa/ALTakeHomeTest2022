...
class DataFileCLIView:
    ...
    def _prompt_choice(self, choices, prompt_msg = None, choice_msg = None, show_choices = True, allow_exit = True):
        """
    ...
    def exit_scene(self):
        ...
        for datafile_index in range(len(datafile_filenames)):
            is_saving_this_datafile = self._prompt_y_n("Unsaved DataFile:\n\t{}\nSave? (y/n): ".format(
        ...
        match selected_command:
            ...
            case "select":
                ...
                if selected_datafile_number in range(1, self._controller.get_total_datafiles() + 1):                
                    ...
                    self._send_message("Selected: {}".format(
                                        self._controller.get_datafile_names(truncate=True)[selected_datafile_index]))
    ...
    def _remove_row(self, searching=False):
        ...
        if searching:
            total_rows = len(self._controller.get_queried_of_current_datafile(self._current_query))
        if total_rows == 0:
            if searching:
        ...
        index_value_selected = self._prompt_choice(range(total_rows), show_choices=False)
        
        if searching:
            dataframe_queried = self._controller.get_queried_of_current_datafile(self._current_query)
        ...
        row_to_remove = self._controller.get_current_datafile_dataframe().loc[[index_to_remove]]
    ...
    def _change_row(self, searching=False):
        ...
        index_value_selected = self._prompt_choice(range(total_rows),
        ...
        row_to_change = self._controller.get_current_datafile_dataframe().loc[[index_to_change]]
        ...
        if confirm_change_row:
            index_of_column_to_change = self._prompt_choice(self._controller.get_current_datafile_columns(),
                                                            choice_msg="Which column are you changing?")
            
            
            if index_of_column_to_change < 0:
                self._send_message("Column selection cancelled. Exiting row change", True)
                return
            
            column_name = self._controller.get_current_datafile_columns()[index_of_column_to_change]
            
            new_value = self._ask_user("New value of {}: ".format(column_name))
            old_value = row_to_change.to_dict('list')[column_name][0]
            
            confirm_change_value = self._prompt_y_n("Confirm \"{}\" -> \"{}\"? (y/n): ".format(old_value,
                                                                                               new_value))
            
            if confirm_change_value:
                self._controller.change_row_of_current_datafile(index_to_change, column_name, new_value)
    ...
    def _display(self, searching=False):
        ...
        display_method_index = self._prompt_choice(all_displays, 
                                                   choice_msg="Choose from the following display methods:")
        ...
        if searching:
            dataframe_to_display = self._controller.get_queried_of_current_datafile(self._current_query, reindex=True)
    ...
    def search_scene(self):
        ...
        search_commands = ["add-search", "remove-search", "remove-row", 
                            "remove-all-rows", "change", "display", "back"]
        ...
        if self._current_query == []:
            ...
            search_result_df = self._controller.get_queried_of_current_datafile(self._current_query, reindex=True)
            ...
            if total_search_rows > 0:
                search_result_df = search_result_df.set_index(pd.Index(list(range(total_search_rows))))
        ...
        search_command_messages = {
            ...
            "remove-search"  : ["Remove a search term", "remove-search", r'remove-search\Z'],
            "remove-row"     : ["Remove a row", "remove-row", r'remove-row\Z'],
            "remove-all-rows": ["Remove all rows in search", "remove-all-rows", r'remove-all-rows\Z'],
            "change"         : ["Change a row", "change", r'change\Z'],
            "display"        : ["Display Data", "display", r'display\Z'],
        ...
        selected_command, user_response = self._select_command(search_command_messages, search_commands)
    ...
    def _add_search_term(self):
        ...
        entered_column = self._controller.get_current_datafile_columns()[index_of_column_to_change]
        
        is_exact = self._prompt_y_n("Exact search? (otherwise substring search) (y/n): ")
    ...
    def _remove_search_term(self):
        ...
        remove_index = self._prompt_choice(self._current_query, choice_msg = "Select a query to remove",
                                           show_choices = False)
    ...
    def _remove_all_rows_in_search(self):
        ...
        """
        search_result_df = self._controller.get_queried_of_current_datafile(self._current_query)
        ...
        confirm_change_row = self._prompt_y_n("Confirm deletion of {} row{}? (y/n): ".format(
        ...
        if confirm_change_row:
            ...
            self._send_message("Removed {} row{}".format(total_rows, "" if total_rows == 1 else "s"), True)
        else:
            self._send_message("Cancelled removal of {} row{}".format(total_rows, 
                                                                       "" if total_rows == 1 else "s"), True)
